...
\include{problem_description.tex}
...
\include{brute_force.tex}

\section{Pattern detection}
\include{pattern_detection.tex}

\subsection{Initial look at the data}
\include{initial_data_analysis.tex}
...
\begin{frame}{Performance optimization}
    \begin{algorithm}[H]
        \begin{algorithmic}[1] % The number tells where the line numbering should start
            \State $F_{\row} \gets \mtx \mtx^T$ \Comment{$\mathcal{O}(N^3)$}
            \State $F_{\col} \gets \mtx^T \mtx$\Comment{$\mathcal{O}(N^3)$}
